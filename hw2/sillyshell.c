...
void parse(char* line, char** argv) {
  ...
  while (*line !=
         '\0')  // outer loop.  keep going until the whole string is read
  ...
  // Heh, I ran out of characters in the input string.  I guess I'm out of
  // tokens. So, whatever slot of the array at argv I'm pointing at?  Yeah, put
  // a NULL there so we can mark the end of entries in the table.
...
int main(int argc, char** argv, char** envp) {
  ...
  while (
    ...
    line[strlen(line) - 1] =
        '\0';  // This is a dirty hack.  Figure it out maybe?
    strncpy(raw_line, line, 1024);

    if ((*line != '\0') &&
        (*line > 31))  // If something was actually typed, then do something...
    {
      // First, get all the addresses of all of the tokens inside the input line

      parse(line,
            line_argv);  //   parse the line to break it into token references

      // Check the first token to see if there are any built in commands
      // we want to handle directly and will do it with an "if/then/else"
      // ladder. if we hit the end of the ladder, we assume the command line was
      // requesting an external program be run as a child process and do
      // that....
      ...
      if (strcmp(line_argv[0], "exit") == 0)
        exit(0);
          ...
          if (strcmp(line_argv[0], "done") == 0)
        exit(0);
      else

          if (strcmp(line_argv[0], "quit") == 0)
        exit(0);
      else
        ...
        // If we got this far, it's not a built-in.  Process the line as if the
        // first token is the name of a program somewhere in the current path
        // environment variable. execute() is the routine that will deal with
        // launching any children.  You'll have to replace my stub code in
        // execute() with something that does the job that's needed.
